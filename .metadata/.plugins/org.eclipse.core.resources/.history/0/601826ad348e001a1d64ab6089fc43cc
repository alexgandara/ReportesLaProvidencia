...
public class v21readXML {
	...
	public static void readXML(String _file_name, 
		...
		//		misParametros.get_ruta_formatos()+"CartaCompletaPruebas.jpg";
	...
	//	String _file_pdf_termica = ".\\data\\20525937195\\05_pdfs\\"+_file+"_termica.pdf";
		...
		try {
			...
			if (Cabecera.get_descripcion_documento().equals("Invoice")) {
				Cabecera.set_tipo_doc_descripcion("FACTURA ELECTRÓNICA");
			...
			try {
				...
				if (Cabecera.get_tipo_doc().substring(1).equals("3")) {
					Cabecera.set_tipo_doc_descripcion("BOLETA ELECTRÓNICA");
					Cabecera.set_Ruc_Dni("    ");
				}
	...
	//		}
			...
			// condicion de pago
			NodeList nList_condicion = doc.getElementsByTagName("cbc:ID");
			Node nNode_condicion = nList_condicion.item(5);
			Cabecera.set_condicion_pago(nNode_condicion.getTextContent());
			System.out.println("Forma de Pago_ __ _ _ _ _ _ _ _ _: " + Cabecera.get_condicion_pago());

			
			// guia
			NodeList nList_guia = doc.getElementsByTagName("cbc:DocumentTypeCode");
			Node nNode_guia = nList_guia.item(0);
			Cabecera.set_guia(nNode_guia.getTextContent());
			System.out.println("GUIA_ _ _ __ _ _ _ _ _ _ _ _: " + Cabecera.get_guia());

			
			
			// fecha_vencimiento
		//	NodeList nList_guia = doc.getElementsByTagName("cbc:DocumentTypeCode");
			Node nNode_fha_vencimientoguia = nList_guia.item(1);
			Cabecera.set_fecha_vencimiento(nNode_fha_vencimientoguia.getTextContent());
			System.out.println("FECHA DE VENCIMIENTO_ _ _ _ _ _: " + Cabecera.get_fecha_vencimiento());


			// dias de vencimiento
		//	NodeList nList_guia = doc.getElementsByTagName("cbc:DocumentTypeCode");
			Node nNode_dias = nList_guia.item(2);
			Cabecera.set_dias(nNode_dias.getTextContent());
			System.out.println("DIAS_ _ _ __ _ _ _ _ _ _ _ _: " + Cabecera.get_dias());


			
			
			// orden de servicio
			NodeList nList_servicio = doc.getElementsByTagName("cbc:ID");
			Node nNode_servicio = nList_servicio.item(2);
			Cabecera.set_servicio(nNode_servicio.getTextContent());
			System.out.println("Orden de Servicio_ _ _ _ _ _: " + Cabecera.get_servicio());

			

			// cbc:CustomerAssignedAccountID "RUC DEL EMISOR"
			NodeList nList_CustomerAssignedAccountID = doc.getElementsByTagName("cbc:ID");
			Node nNode_CustomerAssignedAccountID = nList_CustomerAssignedAccountID.item(4);
			Cabecera.set_ruc_emisor("20547041276");
...
//			Node nNode_CustomerAssignedAccountID_r = nList_CustomerAssignedAccountID_r.item(9);
...
//			System.out.println("RUC del Receptor_ _ _ _ _ _ _ _: " + Cabecera.get_ruc_receptor());
			...
			if (Cabecera.get_descripcion_documento().equals("Invoice")) {
				...
				System.out.println("Sub Total  _ _ _  _ _ _ _ _ _ : " + Cabecera.get_subtotal());
	...
	//		Cabecera.set_seg_nombre("");
			...
			for (int _linea = 0; _linea < nList_linea.getLength(); _linea++) {
				...
				Detalle[_lineaArreglo].set_igv(Detalle[_lineaArreglo].get_subtotal()-Detalle[_lineaArreglo].get_subtotal_sin_igv());
...
*/
					...
					for (int _palabras=0; _palabras<=y-1; _palabras++) {
						...
						if ((_tam_linea+_tam_palabra)<52) {
							...
							if (Detalle[_lineaArreglo-_linea].get_cantidad()==0) {
								Detalle[_lineaArreglo-_linea].set_codigo("999");
							}
	...
	//		v21printPDFConti.imp_factura(_file_xml, Cabecera, Detalle, _lineas_Descripcion,_file_pdf, _file_jpg_para_membrete);

		//	v21printPDF.imp_factura(_file_xml, Cabecera, Detalle, _lineas_Descripcion,_file_pdf, _file_jpg_para_membrete);
